...
PineconeMegaClass::PineconeMegaClass(HttpClient client, char *_code) :
		http(client) {
	code = (char*) calloc(strlen(_code) + 1, 1);
	strcpy(code, _code);
}
String PineconeMegaClass::getDeviceID() {
	return deviceID;
}
String PineconeMegaClass::findDeviceByCode() {
	char path[ 33 + String( code ).length( ) ];
	  strcpy( path, "/rest/m/device/search/codes?code=" );
	  strcat( path, code );
	  Serial.println(path);
	  //@todo
	  // if not extract below method , w5100 will crack,
	  http.beginRequest( );
...
String PineconeMegaClass::createDevice() {
...
String PineconeMegaClass::createVariable(char* name, char* type) {
...
String PineconeMegaClass::createItem(char* value, String variable) {
...
String PineconeMegaClass::makeGetRequest(char* path) {
...
String PineconeMegaClass::makePostRequest(char* path, char* type,
		char* content) {
...
String PineconeMegaClass::fetchDataFromResponse() {
	String response = NULL;
	if (http.contentLength() > 0) {
		while (http.available()) {
			byte data = http.read();
